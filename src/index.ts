...
import { parseUnifiedDiff, filterDiffFiles, buildNumberedPatch, adjustToReviewablePosition } from "./diff";
import { GeminiClient, type AIReview } from "./gemini";
...
export type Dependencies = {
  ...
  createGeminiClient: (apiKey: string, modelName: string) => { review: (prompt: string) => Promise<AIReview[]> };
};
...
export function summarizeComments(comments: ReviewComment[], reviews: AIReview[], fileCount: number): string {
...
export async function run(params: {
  ...
  const filteredFiles = filterDiffFiles(parsedFiles, {
  ...
  const summary = summarizeComments(comments, allReviews, filteredFiles.length);
  ...
  return { skipped: false, summary, comments, reviews: allReviews };
}
